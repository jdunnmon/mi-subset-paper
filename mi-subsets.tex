...
Were fine-grained labels available for every variant of importance, performance on important subsets could be improved by training and evaluating models using this information.  
...
As a result, teams working on medical imaging applications have commonly begun either to leverage semi-automated labelling techniques \citep{Wang2017-vm, Fries2019-ze, Irvin2019-ho, Dunnmon2019-zw, Fries2019-ze} or to apply human expertise to produce a narrow or incomplete set of visual labels \citep{Rajpurkar2017-rc} rather than exhaustively labelling all possible findings and variations; both
of these approaches can yield reduced accuracy on important subsets \citep{Oakden-Rayner2019-yi}.  
...
% The distribution of these subclasses and sensitivity of the original normal-abnormal labels on each are shown in Table \ref{tab:mura2}.
 ...
 We further observe degraded performance on the fracture subclass (0.86), which has subtle discriminative features,  and the degenerative disease subclass (0.76), which Table \ref{tab:mura2} shows to have low-sensitivity labels.  
 These results indicate differences in discriminative feature subtlety and subclass label quality can result in clinically meaningful hidden stratification.
 ...
 \vspace{-5mm}
\centering
\subfloat[]{\includegraphics[trim={0 0 1cm 1.5cm}, clip, width=2.5in]{MURA-ROC.png}}%
