...
In this work, we assess the utility of several possible techniques for measuring and describing hidden stratification effects both on multiple medical imaging datasets and on the well-characterised CIFAR-100 benchmark dataset.
...
% The distribution of these subclasses and sensitivity of the original normal-abnormal labels on each are shown in Table \ref{tab:mura2}.
 ...
 As expected, we also observe degraded AUC for fractures (0.86), a subclass with subtle discriminative features, and for degenerative disease (0.76), which has low-sensitivity superclass labels (Table \ref{tab:mura2}).  
